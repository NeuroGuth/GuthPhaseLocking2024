...
% for iSub = 1:length(subjects)
    ...
    for iSess = 1:size(sessions, 1)
        ...
        if (strcmp(subjects{iSub}, 'TH01') && strcmp({sessions(iSess).name}, 'session_1'))
            continue;
        elseif (strcmp(subjects{iSub}, 'TH03') && strcmp({sessions(iSess).name}, 'session_0'))
            continue;
        end
            ...
            for iClus = 1:max(t.cluster_class(:, 1))
                ...
                for iCond = 1:size(param.condition, 1)
                    ...
                    for iSur = 1:param.nSur
                        ...
                        % get surrogate mean resultant vector
                        try
